...
from ...models import UserInToken, RequestResponse
...
async def get_request(
    ...
    if current_user.sub != request.get("user_id") and current_user.role != "admin":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You can only access your own requests or require admin privileges.",
        )
...
@router.get(
    ...
    description="Lista requisições. Admins podem ver todas e filtrar por utilizador; utilizadores normais veem apenas as suas.",
...
@router.get(
    ...
    description="Obtém requisições num intervalo de datas (YYYY-MM-DD). Admins podem ver todas; utilizadores apenas as suas.",
